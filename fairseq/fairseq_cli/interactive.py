    ...
    if args.constrained_decoding:
        ...
        src_tokens = [
            task.source_dictionary.encode_line(
                encode_fn(src_str.split(separator)[0]), add_if_not_exist=False, prepend_bos=args.prepend_bos
            ).long()
        ...
        # read constraints
        # possible unnecessary tokenization.
        constraint_tokens = [
    ...
    for inputs in buffered_read(args.input, args.buffer_size):
        ...
        for batch in make_batches(inputs, args, task, max_positions, encode_fn):
            ...
            for i, (id, hypos) in enumerate(zip(batch.ids.tolist(), translations)):
                if len(src_tokens) == 3:
                    src_tokens_i = utils.strip_pad(src_tokens[0][i], tgt_dict.pad())
                else:
