...
def _apply_oracle_ins_masks(
    in_tokens, out_tokens, padding_idx, unk_idx, eos_idx
):
    # temp value to replace unk
    with torch.no_grad(): 
        # dev includes unk tokens
        in_tokens[in_tokens.eq(unk_idx)] = 35000
        out_tokens[out_tokens.eq(unk_idx)] = 35000
        masked_tgt_masks, _, mask_ins_targets = _get_advanced_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx)
        int_out = _apply_ins_masks(in_tokens, None, None, mask_ins_targets, padding_idx, unk_idx, eos_idx)[0] 
        int_mask = int_out.eq(unk_idx)
        # if not torch.all(int_mask.sum(axis=1).eq(masked_tgt_masks.sum(axis=1))):
            # print(masked_tgt_masks.sum(axis=1), int_mask.sum(axis=1))
        int_out.masked_scatter_(
            int_mask, out_tokens[masked_tgt_masks]
        )
        int_out[int_out.eq(35000)] = unk_idx
        in_tokens[in_tokens.eq(35000)] = unk_idx
        out_tokens[out_tokens.eq(35000)] = unk_idx
    return int_out

def _get_advanced_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx, eos_idx=None):
    libnat, use_cuda = load_libnat()

    def _get_ins_targets_cuda(in_tokens, out_tokens, padding_idx, unk_idx, eos_idx=None):
    ...
    def _get_ins_targets_cpu(in_tokens, out_tokens, padding_idx, unk_idx, eos_idx=None):
        in_seq_len, out_seq_len = in_tokens.size(1), out_tokens.size(1)
...
def _apply_ins_masks(
    ...
    in_tokens_tmp = in_tokens.detach().clone()
    in_masks = in_tokens_tmp.ne(padding_idx)
    in_lengths = in_masks.sum(1)

    # HACK: hacky way to shift all the paddings to eos first.
    in_tokens_tmp.masked_fill_(~in_masks, eos_idx)
    ...
    out_tokens = (
        in_tokens_tmp.new_zeros(in_tokens_tmp.size(0), out_max_len)
        .fill_(padding_idx)
        .masked_fill_(out_masks, unk_idx)
    )
    out_tokens[:, 0] = in_tokens_tmp[:, 0]
    out_tokens.scatter_(1, reposition, in_tokens_tmp[:, 1:])
