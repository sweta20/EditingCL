...
class LevenshteinTransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        if random.random() < self.decoder.prob: # insertion ->deletion
            ...
            # make online prediction : Is differntiable?
            word_predictions = F.log_softmax(word_del_out, dim=-1).max(-1)[1].bool()

            word_predictions, _, _, _ = _apply_del_words(
                prev_output_tokens,
                None,
                None,
                None,
