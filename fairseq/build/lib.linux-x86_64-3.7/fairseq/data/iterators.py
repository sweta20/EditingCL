...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def __init__(
        ...
        num_workers=0, epoch=1, pacing="no_pacing", warmup_iters=5000, num_updates=0,
        validation=False, 
    ):
    ...
    def next_epoch_itr(self, shuffle=True, fix_batches_to_gpus=False, num_updates=None):
        """Return a new iterator over the dataset.
        ...
        self.dataset.set_epoch(self.epoch)
        self.dataset.set_num_updates(num_updates)
    ...
    def root(self, bias=0.25, p=2):
        return min([1, np.sqrt(self.num_updates * ((1 - bias**2) / self.warmup_iters) + bias**2)])

    def linear(self, bias=0.05):
        return min([1, (1 / self.warmup_iters) * self.num_updates + bias])
    ...
    def _get_iterator_for_epoch(self, epoch, shuffle, fix_batches_to_gpus=False, offset=0):
        ...
        def shuffle_batches(batches, seed):
            ...
            return batches
        if self.pacing == "no_pacing" or self.validation:
            pacing = 1
        elif self.pacing == "root":
            pacing = self.root()
        elif self.pacing == "linear":
            pacing = self.linear()
        pacing_idx = int(pacing * len(self.frozen_batches))
        print("Number of batches (percent data): ", float(pacing_idx)/ len(self.frozen_batches))
        if self._supports_prefetch:
            batches = self.frozen_batches[:pacing_idx]
            ...
            if shuffle:
                batches = shuffle_batches(list(self.frozen_batches[:pacing_idx]), self.seed + epoch)
            else:
                batches = self.frozen_batches[:pacing_idx]
            batches = list(ShardedIterator(
                batches, self.num_shards, self.shard_id, fill_value=[]
